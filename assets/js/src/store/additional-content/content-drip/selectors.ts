...
export interface AdditionalContentStoreStateWithContentDrip {
  contentDripSettings: Record<number, ContentDripItemSettings>;
  contentDripLoading: Record<number, boolean>;
  contentDripErrors: Record<number, string | null>;
  contentDripSaving: Record<number, boolean>;
  contentDripSaveErrors: Record<number, string | null>;
  prerequisites: Record<number, PrerequisitesByTopic[]>;
  prerequisitesLoading: Record<number, boolean>;
  prerequisitesErrors: Record<number, string | null>;
}

// Selectors
export const getContentDripSettings = (state: any, postId: number): ContentDripItemSettings | null => {
  return state.contentDripSettings?.[postId] || null;
};

export const isContentDripLoading = (state: any, postId: number): boolean => {
  return state.contentDripLoading?.[postId] || false;
};

export const getContentDripError = (state: any, postId: number): string | null => {
  return state.contentDripErrors?.[postId] || null;
};

export const isContentDripSaving = (state: any, postId: number): boolean => {
  return state.contentDripSaving?.[postId] || false;
};

export const getContentDripSaveError = (state: any, postId: number): string | null => {
  return state.contentDripSaveErrors?.[postId] || null;
};

export const getPrerequisites = (state: any, courseId: number): PrerequisitesByTopic[] | null => {
  return state.prerequisites?.[courseId] || null;
};

export const isPrerequisitesLoading = (state: any, courseId: number): boolean => {
  return state.prerequisitesLoading?.[courseId] || false;
};

export const getPrerequisitesError = (state: any, courseId: number): string | null => {
  return state.prerequisitesErrors?.[courseId] || null;
};
